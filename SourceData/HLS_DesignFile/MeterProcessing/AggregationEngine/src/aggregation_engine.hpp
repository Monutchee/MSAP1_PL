...
//
// Why one engine rather than a shared block: HLS cannot share an
// instance across IP boundaries, so the only way two tiers can use one
// finalize datapath is to live in one component. Two consequences fall
// out, both good:
