...
# The module reference must expose M_AXIS_SCYC first (build_bd.tcl also
...
set scyc_port [get_bd_intf_pins -quiet $meter_core/M_AXIS_SCYC]
if {$scyc_port eq ""} { error "M_AXIS_SCYC did not appear on the module reference" }

if {[get_bd_cells -quiet ${meter_core}/../SCYC_record_fifo] ne "" ||
    ...
    set fifo [create_bd_cell -type ip \
        -vlnv [get_property VLNV $template_fifo] $buffer_group/SCYC_record_fifo]
    ...
    set group_pin [create_bd_intf_pin -mode Slave \
        -vlnv xilinx.com:interface:axis_rtl:1.0 $buffer_group/S_AXIS_SCYC]
    connect_bd_intf_net $scyc_port $group_pin
    connect_bd_intf_net [get_bd_intf_pins $buffer_group/S_AXIS_SCYC] \
    ...
    validate_bd_design
    save_bd_design
    puts "SCYC record path added: $fifo -> $switch_cell/$new_si"
}

if {!$already_open} { close_project }
puts "add_single_cycle_record_path done"
