...
-- Test-only shim: exposes meter_cycle_aggregator_hls_shim's basic-result
-- record port as scalar ports so the SystemVerilog equivalence testbench
-- can drive it (mixed-language boundaries cannot carry VHDL records).
-- Not part of the production hierarchy; the production instantiation is
-- the shadow instance inside meter_core.
entity meter_cycle_aggregator_hls_tbshim is
  port (
    aclk    : in std_logic;
...
architecture rtl of meter_cycle_aggregator_hls_tbshim is
...
begin
  ...
  aggregator : entity work.meter_cycle_aggregator_hls_shim
    port map (
      aclk => aclk,
      aresetn => aresetn,
